...
역할:
- data/dummy_data.py 에 정의된 더미 7일 활동 데이터(DUMMY_ACTIVITY_7D)를 기반으로
  MemeScore v1.1을 계산하고,
- 각 지표별 점수/구간을 분해해서 보여주며,
- LLM(OpenAI)을 사용해 자연어 설명을 생성하는 스크립트.

특징:
- 실제 MemeX posts API가 비어 있어도, 더미 데이터만으로 데모 가능
- 입력: creator_id (username 또는 "username#tag")
"""
...
from api import get_mock_user_data, get_user_basic

# ------------------------------------------------------------
# 0. data/dummy_data.py import 세팅
# ------------------------------------------------------------
...
try:
    from dummy_data import DUMMY_ACTIVITY_7D
except ImportError as e:
    raise ImportError("data/dummy_data.py 에서 DUMMY_ACTIVITY_7D 를 가져올 수 없습니다.") from e
...
# ------------------------------------------------------------
# 2. creator_id → (username, userNameTag) 매핑
# ------------------------------------------------------------

def resolve_username_and_tag(creator_id: str) -> Tuple[str, str]:
    """
    - "username#tag" 형식이면 그대로 분리
    - 아니면 mock-user-data 에서 userName == creator_id 인 유저를 찾아 tag 사용
    """
    if "#" in creator_id:
...
def calc_bot_score(metrics: Dict[str, Any]) -> float:
    ...
    # 조회수 대비 좋아요 비율이 너무 높은 경우
    if like_view > 0.3:
        score += 0.4
    if like_view > 0.5:
        score += 0.2

    # 좋아요는 많은데 댓글/리포스트 거의 없는 경우
...
# ------------------------------------------------------------
# 5. 더미 데이터 기반 메트릭 가져오기
# ------------------------------------------------------------

def fetch_creator_metrics(creator_id: str) -> Dict[str, Any]:
    """
    data/dummy_data.py 의 DUMMY_ACTIVITY_7D 를 기반으로
    해당 크리에이터의 7일치 활동 지표 + MemeScore 를 계산한다.
    """
    username, tag = resolve_username_and_tag(creator_id)

    if username not in DUMMY_ACTIVITY_7D:
        raise ValueError(
            f"username '{username}' 에 대한 더미 데이터가 없습니다. "
            "dummy_data.py 의 DUMMY_ACTIVITY_7D 에 해당 유저를 추가하세요."
        )

    # 유저 프로필(표시 이름)만 API에서 가져오고, 활동숫자는 더미에서 사용
    user_info = get_user_basic(username, tag)

    dummy = DUMMY_ACTIVITY_7D[username]

    likes_7d      = float(dummy["likes_7d"])
    comments_7d   = float(dummy["comments_7d"])
    reposts_7d    = float(dummy["reposts_7d"])
    quotes_7d     = float(dummy["quotes_7d"])
    views_7d      = float(dummy["views_7d"])
    followers     = float(dummy["followers"])
    tip_count_7d  = float(dummy["tip_count_7d"])
    tip_amount_7d = float(dummy["tip_amount_7d"])
...
def build_creator_context(metrics: Dict[str, Any], bot_score: float) -> str:
    ...
    lines = [
        ...
        f"- 오늘 MemeScore: {metrics.get('memescore_v1_1', 0.0):.2f}",
        "",
        "최근 7일 활동 요약:",
...
def explain_meme_score(creator_id: str) -> str:
    ...
    if warnings:
        report_bug(
            title="Creator metrics anomaly (dummy_explainer)",
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("사용법: python dummy_explainer.py <creator_id 또는 username#tag>")
        print("예시:  python dummy_explainer.py zIfjpicANr")
        raise SystemExit(1)

    cid = sys.argv[1]
    try:
        result = explain_meme_score(cid)
        print("\n===== MemeScore 설명 (dummy_explainer) =====\n")
        print(result)
    except Exception as e:
        report_bug(
            title="dummy_explainer CLI error",
